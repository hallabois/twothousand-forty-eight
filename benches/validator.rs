...
fn validate_history(c: &mut Criterion) {
    ...
    c.bench_function("validate_first_move", |b| b.iter(|| bvalidate_history(history.clone())));
}

fn validate_first_move(c: &mut Criterion) {
	let history = parser::parse_data(String::from(GAMEOBSCENE)).unwrap();
    c.bench_function("validate_first_move", |b| b.iter(|| bvalidate_first_move(history.clone())));
}
